...
<img src="https://github.com/satrajit-chatterjee/motion_planning_demo/blob/master/assets/video.gif" width=80% height=80%>

In the above GIF the green line is the path generated by the planner. The blue line represents the final waypoints which are created for the controller after having generated intermediate waypoints between the pairs of original waypoints. The full video can be found [here](https://github.com/satrajit-chatterjee/motion_planning_demo/blob/master/assets/video.mp4).

### 5. Algorithm - RRT*
The RRT* (Rapidly-exploring Random Tree Star) algorithm is a path planning algorithm that is guaranteed to converge to a globally optimal solution as the 
number of iterations approaches infinity. It constructs a tree of states, starting from the initial state, and incrementally growing the tree by adding new states to it. The algorithm repeatedly samples random states within the configuration space of the agent, and extends the tree towards that state. It continues 
...
2. Clone repositories in the src/ directory

```
cd src
git clone git@github.com:satrajit-chatterjee/motion_planning_demo.git
git clone git@github.com:satrajit-chatterjee/map_generator.git
git clone git@github.com:satrajit-chatterjee/kr_mav_control.git
```
...
```
roslaunch sim_manager sim_manager.launch
```
